...
function AgencyMark({ logoUrl, name, color = DEFAULT_BRAND, size = 32 }: {
  logoUrl?: string; name?: string; color?: string; size?: number;
}) {
  if (logoUrl) {
    return <img src={logoUrl} alt="" style={{ width: size, height: size, objectFit: "contain", borderRadius: 6 }} />;
  }
...
function GalleryInner({ agencySlug }: { agencySlug: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (first) {
          const userId = (first.data() as { userId?: string }).userId;
          if (userId) {
            ...
            if (userSnap.exists()) {
              ...
              setAgency({
                ...
                language:   u.language || first.data().language || "en",
              });
            }
          }
        }
      } catch (err) {
        console.error("[CustomDomainGallery] Firestore error:", err);
      } finally {
        setLoading(false);
      }
    };
              ...
              return (
                ...
                >
                  ...
                  <div style={{ padding: "18px 20px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); router.push(`/${pkg.id}`); }}
...
export default function CustomDomainGallery({ agencySlug }: { agencySlug: string }) {
  return (
    <Suspense fallback={null}>
      <GalleryInner agencySlug={agencySlug} />
    </Suspense>
  );
}
